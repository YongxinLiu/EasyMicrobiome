...
if (TRUE){
  option_list = list(
    make_option(c("-i", "--input"), type="character", default="result/metaphlan4/taxonomy.spf",
    ...
    #make_option(c("-t", "--type"), type="character", default="bar",
    #            help="specify plot type, e.g. heatmap, bar, [default %default]"),
...
# taxonomy = read.table("taxonomy_modified.spf",header = T,row.names = NULL,
#                       sep = "\t",quote = "",check.names = F)
...
annotation_file2 <- paste0(out,"graphlan_annotate_heatmap.txt")
write.table(global, annotation_file2,sep="\t",quote = F,col.names = F,row.names = F, na="")
write.table(label_color, annotation_file2, sep = "\t",append=T, quote = F,col.names = F,row.names = F, na="")

annotation_file3 <- paste0(out,"graphlan_annotate_bar.txt")
write.table(global, annotation_file3,sep="\t",quote = F,col.names = F,row.names = F, na="")
write.table(label_color, annotation_file3, sep = "\t",append=T, quote = F,col.names = F,row.names = F, na="")
...
if (type == "heatmap"){
  ...
  for (i in 1:length(group)){
    ...
    write.table(temp, file=annotation_file2, append = T, sep = "\t", quote=F, row.names=F, col.names=F)
    ...
    write.table(temp_data,file=annotation_file2, append = T, sep = "\t",quote = F,col.names = F,row.names = T, na="")
    col_names
  ...
  for (i in 1:length(group)){
    ...
    #write.table("#", file="graphlan_annotate.txt", append = T, quote=F, eol = "", row.names=F, col.names=F)
    write.table(temp, file=annotation_file3, append = T, sep = "\t", quote=F, row.names=F, col.names=F)
    temp_data <- data.frame(matrix(ncol=0,nrow=nrow(tax_mean)))
    temp_data$"ring_height" <- rep("ring_height")
    temp_data$"ring_number" <- rep(i)
    temp_data$"height" <- tax_mean[,group[i]]
    row.names(temp_data) <- row.names(tax_mean)
    write.table(temp_data,file=annotation_file3, append = T, sep = "\t",quote = F,col.names = F,row.names = T, na="")
    col_names
